-- 030_usage_plan_v2.sql
-- Plan v2: extraction_calls, embed_tokens_used, atomic cap RPC.
-- Adds columns to usage_daily and bump_usage_if_within_cap RPC.

set search_path = public;

-- New columns (default 0 for existing rows)
alter table usage_daily
  add column if not exists extraction_calls int not null default 0,
  add column if not exists embed_tokens_used int not null default 0;

-- Atomic cap RPC: only bumps if all new totals would be within caps.
-- Caps are passed in by the Worker (from plan resolution).
-- Returns: on success, table row; on cap exceeded, single row with exceeded = true and limit_name set.
...
begin
  ...
  select * into v_row
  from usage_daily
  where usage_daily.workspace_id = p_workspace_id and usage_daily.day = p_day
  for update;
