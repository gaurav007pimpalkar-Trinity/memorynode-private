...
- **Memories** — text and optional metadata you send via the API; chunked and embedded for search. Scoped to your **workspace** and the **`user_id`** / **`namespace`** you provide.
- **API keys** — stored as hashes; the dashboard shows only a **prefix** after creation.
- **Dashboard sessions** — short-lived rows used to bind your browser session to a workspace (see [SECURITY.md](./SECURITY.md)).
...
- **Export** — use documented export paths for your workspace where available ([external README](./external/README.md)).
- **Revoke keys** — create and revoke API keys from the console to cut off access.

## Retention in practice

- **Memories** persist until **you delete them** or the **workspace** is closed and data is removed per your agreement with MemoryNode (self-hosted or hosted service terms).
