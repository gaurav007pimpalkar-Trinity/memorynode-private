...
Recent backend hardening keeps API behavior unchanged while making optional internals fail-safe in degraded or stubbed environments: learned-adjustment lookup, monthly LLM usage reads, retrieval-attribute loading, and async feedback persistence now no-op safely when dependent storage/query capabilities are unavailable.
...
On the **public** Worker, `CONTROL_PLANE_ORIGIN` and `CONTROL_PLANE_SECRET` apply only to proxying **`/v1/admin/*`** to that control-plane Worker. If either is unset, **`/v1/admin/*`** returns **503** with `CONFIG_ERROR` explaining the missing vars. **`/healthz`**, **`/ready`**, **`/v1/health`**, and tenant **`/v1/*`** routes do not require those two variables (so minimal staging deployments can omit them until admin proxy is needed).
